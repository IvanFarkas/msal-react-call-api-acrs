...
class Todo {
  ...
  static getTodos(owner) {
    return db.get('todos').filter({ owner: owner }).value();
  }

  static getTodo(owner, id) {
    return db.get('todos').filter({ owner: owner }).find({ id: id }).value();
  }

  static updateTodo(owner, id, todo) {
    db.get('todos').find({ owner: owner, id: id }).assign(todo).write();
  }
