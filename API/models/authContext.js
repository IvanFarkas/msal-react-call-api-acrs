...
class AuthContext {
  ...
  static getAuthContexts() {
    return db.get('acrs').value();
  }

  static getAuthContext(authContextId) {
    return db.get('acrs').find({ authContextId: authContextId }).value();
  }

  static updateAuthContext(authContextId, acrs) {
    db.get('acrs').find({ authContextId: authContextId }).assign(acrs).write();
  }
  ...
  static deleteAuthContext(authContextId) {
    db.get('acrs').remove({ authContextId: authContextId }).write();
  }
}
